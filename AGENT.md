...
Every tool call must write a log entry to the daily logfile `logs/exec_YYYYMMDD.log` (e.g., `logs/exec_20260203.log`). Logrotate is daily—if the date changes, create a new logfile automatically. Log entries should include:
- Timestamp
- Tool/script name
- Input parameters
...
- `logs/` - Daily execution logs (`exec_YYYYMMDD.log` format, logrotate daily)
...
Be pragmatic. Be reliable. Self-anneal. Log everything.